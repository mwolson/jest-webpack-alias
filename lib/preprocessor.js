...
function resolveFirstDir(dirname, rest) {
  ...
  if (rest) {
    ...
    matchingDir = find(moduleDirs, function(aliasDir) {
      return -1 !== _.findIndex(fileExtensions, function(ext) {
        matchingExt = ext;
        return dirHas(aliasDir, dirname + ext);
      });
    });
    if (matchingDir) {
      matchingDir += matchingExt;
    }
  }
...
function resolve(dependency, filename) {
  ...
  var matchingDir = path.join(matchingFirstDir, dirname);
